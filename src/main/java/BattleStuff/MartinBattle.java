...
public class MartinBattle implements BattleBase {
    int count = 0;
    ...
    public boolean MartinFight(MainCharacter newPastor, Monster monster, ParentVariable defeated) {
        Monster calvin = new Monster(
                "Calvin",
                100,
                10,
                6,
        ...
        while (newPastor.getHealth() > 0 && monster.getHealth() > 0) {
            ...
            if (rand.nextInt(100) < 50) {
                System.out.println("Calvin pops out from behind and attacks as well");
                MartinAttack(newPastor, monster);
                CalvinAttack(newPastor, calvin);
            } else {
                MartinAttack(newPastor, monster);
            }
            newPastor.setDefense(newPastor.getRealDefense());
            if (newPastor.getTotalHealth() <= 0) {
                defeated.martinDefeated = false;
                break;
            }
        ...
        scanner.close();
        if (!defeated.martinDefeated) {
            System.out.println("HHAHAHHAHA");
        }
        return false;
    }

    public void MartinAttack(MainCharacter newPastor, Monster mon) {
        int totalDamage = mon.getDamage();

        Random rand = new Random();
        int x = rand.nextInt(100);
        if (x < mon.getCritChance()) {
            System.out.println("Martin : Yeah... Im good (hits a crit!)");
